...
| `<leader>g?` | Open this doc | Opens `FUGITIVE.md` in a new read-only tab | Quick in-editor reference while working |
...
| `<leader>gc` | Commit (verbose) | Opens `:Git commit -v` | Create a commit with staged diff visible in commit buffer |
...
3. Optional per-file review: `,gd` to inspect a split diff for the current file.
4. `,gc` to commit (verbose mode shows staged diff while writing the message).
5. `,gp` to push.
...
3. Optional per-file review: `,gd` to verify exactly what changed in this file.
4. `,gc` to commit only what is staged.
...
3. Optional per-file deep review with `,gd`.
